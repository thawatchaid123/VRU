...
const Check = () => {
        ...
        try {
            ...
            if (data.success === true && data.reports && data.reports.length > 0) {
                ...
                data.reports.forEach(report => {
                    console.log(`Report ID: ${report.id}, Status: ${report.status}, Type: ${report.type}`);
                    initialRatingState[report.id] = {
                        rating: report.rating || 0,
                        comment: report.comment || '',
                        type: report.type, // Store type for rating submission
                        isSubmitting: false,
                        saved: !!report.rating
                    };
                });
                ...
                setError(data.error || 'ไม่พบข้อมูลการแจ้งซ่อมหรือปัญหาสิ่งแวดล้อมสำหรับเบอร์โทรนี้');
            }
    ...
    const handleRatingSubmit = async (reportId) => {
        const reportState = ratingState[reportId] || {};
        const { rating, comment, type } = reportState;

        if (!rating || rating < 1 || rating > 5) {
            setRatingError(prev => ({ ...prev, [reportId]: 'กรุณาเลือกคะแนนดาว (1-5)' }));
            return;
        }

        if (!type || !['repair', 'environment'].includes(type)) {
            setRatingError(prev => ({ ...prev, [reportId]: 'ไม่พบข้อมูลประเภทรายการ (repair/environment)' }));
            return;
        }
        ...
        try {
            console.log('Submitting rating:', { report_id: reportId, rating, comment, type });
            ...
            if (!response.ok || !data.success) {
                throw new Error(data.message || 'ไม่สามารถบันทึกคะแนนได้');
            }
    ...
    }, [selectedImage, isDragging, zoomLevel, position.x, position.y]);

    return (
        <div className="complaints-form">
            <div className="complaints-card">
                <h2 className="header">ตรวจสอบสถานะการแจ้ง</h2>
            ...
            {!isLoading && result && result.length > 0 && (
                <div className="results-section">
                    ...
                    {result.map((item, index) => {
                        ...
                        console.log(`Rendering Report ID: ${item.id}, Is Completed: ${isCompleted}, Type: ${item.type}`);
                        return (
                            <div key={item.id || `report-${index}`} className="complaints-card result-card">
                                <h4 className="result-header">รายการแจ้ง #{index + 1} </h4>
                                <div className="info-grid">
                                    <div className="info-group">
                                        <strong>เบอร์โทร :</strong> {item.phone ?? 'N/A'}
                                    </div>
                                    <div className="info-group">
                                        <strong>{item.type === 'environment' ? 'ปัญหาสิ่งแวดล้อม :' : 'สิ่งชำรุด :'}</strong> 
                                        {item.repair_name ?? 'N/A'} {item.location_name ?? 'N/A'}
                                    </div>
                                    <div className="info-group">
                                    ...
                                    {item.image && (
                                        <div className="info-group image-group">
                                            ...
                                            <img
                                                ...
                                                alt={`รูปภาพ ${item.repair_name || (item.type === 'environment' ? 'แจ้งปัญหาสิ่งแวดล้อม' : 'แจ้งซ่อม')}`}
            ...
            {!isLoading && !result && error && (error === 'ไม่พบข้อมูลการแจ้งซ่อมหรือปัญหาสิ่งแวดล้อมสำหรับเบอร์โทรนี้' || error.toLowerCase().includes("ไม่พบข้อมูล")) && (
                <div className="complaints-card">
